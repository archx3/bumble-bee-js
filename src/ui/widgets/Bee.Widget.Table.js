 ...
 *    @requires {@link Barge.Widget} aka DomCore
 *    @requires {@link Barge.Event}
 *
 *   @requires {@link Barge.Widget.TableSorter}
 *
...
})(typeof window !== undefined ? window : this, function factory(window)
{
   ...
   function Table(tableEl, options)
   {
      ...
      Bu.assert(Bu.defined(window) && Bu.defined(document),
         'Barge.Widget.Table requires a window object as host');
      ...
      this.tableEl = Bu.isString(tableEl) ? Bee.Widget.getEl(tableEl) : tableEl;

      //ensure that the table element is available
      Bu.assert(Bu.defined(this.tableEl),
                '@param tableEl is missing; Bee.Widget.Table requires a table element to manage',
                function ()
                {
                   for(let i in self)
                   {
   ...
   {
      let self = this,
          tSorter = new Bee.Widget.TableSorter(this.tableEl, {rowStepping : self.options.sortingRowStepping});
