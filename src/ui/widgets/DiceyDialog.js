 ...
 *   <-(____)->
 *    \      /
 * @fileOverview contains instruction[code] for creating Dialog Boxes
 * Dialog boxes cuurently supported include
 * Alerts
 * Confirm dialogs,
 * prompts,
 * File dialogs
 * Generic Dialogs [Whose content is created from a user supplied HTML root-node Element]
 * A replacement for the LEGACY Bee.Dialog which is now deprecated in favour of this
 *
 * @requires {@link Bee.Utils}
 * @requires {@link Bee.String}
 * @requires {@link Bee.Widget}
 * @requires {@link Bee.Widget.Overlay}
 *
 */
...
})(typeof window !== undefined ? window : this, function factory(window)
{
   ...
   /**
    * a utility function for setting aliases on config properties
    * @param object {Object}
    ...
    * @class
    * @extends Widget
    * At it's current implementation it does not require extending the base Widget class
    * but in the future when we add support for turning HTML from a source node into
    * a dialog widget, then we'll need to extend the properties of the Widget class
    */
   class DiceyDialog extends Widget {
      /**
       *
    ...
    * highlightDefaultBtn: boolean}}
      ...
      progressDialog(options = {})
      {
         ...
         options["dismissOnOverlayClick"] = !!(Bu.defined(options["dismissible"]) && options.dismissible === true);
    ...
    *    String<"a|w|q|i|f|s"> , i : String<"w|q|i|f"> }}
     ...
     affirmBtnTitle : String, cancelBtnTitle : String, primaryBtnNumber : Number, icon : String<"w|q|i|f|a|s|p">, dismissible : Boolean}}
       */
      render(isAlert = true, options = null, isFileDialog = false)
      {
         let self = this;
         //console.log("here");
         ...
         this.overlay = new Bee.Widget.Overlay({
                                                  opacity         : .7,
                                                  backgroundColor : "",
                                                  dismissOnClick  : options["dismissOnOverlayClick"],
       ...
       */
      createTitle(title = "", hasPrompt = false)
      {
         let self = this;
       ...
       * @param isProgDialog
       */
      createBody(options, isPrompt = false, isFileDialog = false, isProgDialog = false)
      {
       ...
       * @returns {*|Element}
       */
      createFoot(isAlert = true, options = {}, isFileDialog = false)
      {
   ...
   Bee.DiceyDialog = {
       ...
       *    cancelBtnNumber : Number, icon : String<"w|q|i|f"> , i : String<"w|q|i|f">, primaryBtnN : Number, files : Array<{ name :
       *    String, type : String, iconClass : String}>, accept : String, , dismissOnOverlayClick : Boolean, defaultFileName}}
       * @param bodyContent {HTMLElement}
       */
      generic : function(options, bodyContent)
      {
         //
      },
