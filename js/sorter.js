/**
 * Created by ARCH on 29/07/2016.
 */
/**
 sortable
 @version 2
 7th April 2007
 @Author Stuart Langridge, http://www.kryogenix.org/code/browser/sortable/
 sil@kryogenix.org{}
...
(function ()
{
   ...
   var the = null,
       ...
       sortrevind = null,
       sortfwdind = null,
       sortbottomrows = null, aa, newrows, bb, y, dt2, dt1, m, d;
   ...
   var sortable = {
      ...
      makeSortable: function(table) {
         ...
         for (var j=0; j<table.rows.length; j++) {
            if (table.rows[j].className.search(/\bsortbottom\b/) != -1) {
      ...
      guessType: function(table, column) {
         ...
         for (var i = 0, sortfn, possdate, text; i<table.tBodies[0].rows.length; i++) {
         ...
         while(swap) {
            ...
            for(var i = b; i < t; ++i) {
               if ( comp_func(list[i], list[i+1]) > 0 ) {
                  var d = list[i]; list[i] = list[i+1]; list[i+1] = d;
                  swap = true;
               }
            } // for
            t--;
            ...
            for(var c = t; c > b; --c) {
               if ( comp_func(list[c], list[c-1]) < 0 ) {
                  var q = list[c]; list[c] = list[c-1]; list[c-1] = q;
                  swap = true;
               }
            } // for
            b++;
...
// with srcInputEl from Tino Zijdel, Matthias Miller, Diego Perini
